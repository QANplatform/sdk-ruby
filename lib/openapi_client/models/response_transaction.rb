  ...
  class ResponseTransaction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ResponseTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ResponseTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'access_list')
        self.access_list = attributes[:'access_list']
      end
    ...
    def hash
      [access_list, block_hash, block_number, chain_id, from, gas, gas_price, hash, input, max_fee_per_gas, max_priority_fee_per_gas, nonce, r, s, to, transaction_index, type, v, value].hash
    end
